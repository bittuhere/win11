...
export const Explorer = () => {
  ...
  const selItem = selected ? files.data.getId(selected) : null;
  ...
  return (
    ...
    >
      ...
      <div className="windowScreen flex flex-col">
        <Ribbon
          selected={selItem}
          onNewFolder={() => dispatch({ type: "FILEMKDIR" })}
          onNewFile={() => dispatch({ type: "FILEMKFILE" })}
          onCut={() => selected && dispatch({ type: "FILECLIP", payload: selected, mode: "cut" })}
          onCopy={() => selected && dispatch({ type: "FILECLIP", payload: selected, mode: "copy" })}
          onPaste={() => dispatch({ type: "FILEPASTE" })}
          onRename={() => {
            if (!selItem) return;
            const name = window.prompt("Rename", selItem.name);
            if (name) dispatch({ type: "FILEREN", payload: { id: selItem.id, name } });
          }}
          onDelete={() => selected && dispatch({ type: "FILEDEL", payload: selected })}
          onShare={() => selected && dispatch({ type: "EDGELINK", payload: "https://onedrive.live.com" })}
        />
        <div className="restWindow flex-grow flex flex-col">
          ...
          <div className="sec2">
            <NavPane />
            <ContentArea searchtxt={searchtxt} selected={selected} setSelect={setSelect} />
          </div>
          <div className="sec3">
...
const ContentArea = ({ searchtxt, selected, setSelect }) => {
  const files = useSelector((state) => state.files);
  const special = useSelector((state) => state.files.data.special);
  const fdata = files.data.getId(files.cdir);
  const dispatch = useDispatch();
  ...
  const handleKey = (e) => {
    if (e.key == "Backspace") {
      ...
      const item = files.data.getId(selected);
      const name = window.prompt("Rename", item?.name || "");
      if (name) dispatch({ type: "FILEREN", payload: { id: selected, name } });
    }
  };
...
const Ribbon = ({ onNewFolder, onNewFile, onCut, onCopy, onPaste, onRename, onDelete, onShare }) => {
  const [menu, setMenu] = useState(false);
  const dispatch = useDispatch();
  return (
    <div className="msribbon flex">
      <div className="ribsec" style={{ position: "relative" }}>
        <div className="drdwcont flex handcr" onClick={() => setMenu((v) => !v)}>
          <Icon src="new" ui width={18} margin="0 6px" />
          <span>New</span>
        </div>
        {menu && (
          <div className="explorerMenu">
            <button type="button" onClick={() => { setMenu(false); onNewFolder(); }}>Folder</button>
            <button type="button" onClick={() => { setMenu(false); onNewFile(); }}>Text document</button>
          </div>
        )}
      </div>
      <div className="ribsec">
        <Icon src="cut" ui width={18} margin="0 6px" onClick={onCut} />
        <Icon src="copy" ui width={18} margin="0 6px" onClick={onCopy} />
        <Icon src="paste" ui width={18} margin="0 6px" onClick={onPaste} />
        <Icon src="rename" ui width={18} margin="0 6px" onClick={onRename} />
        <Icon src="share" ui width={18} margin="0 6px" onClick={onShare} />
        <span className="handcr" style={{ marginLeft: 8 }} onClick={onDelete}>Delete</span>
      </div>
      <div className="ribsec">
        <div className="drdwcont flex handcr" onClick={() => dispatch({ type: "FILEVIEW", payload: 1 })}>
          <Icon src="sort" ui width={18} margin="0 6px" />
          <span>Sort</span>
        </div>
        <div className="drdwcont flex handcr" onClick={() => dispatch({ type: "FILEVIEW", payload: 5 })}>
